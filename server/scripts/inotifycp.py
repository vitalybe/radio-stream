#!/usr/bin/env python
# Copied from: https://gist.github.com/wernight/11401031
...
Usage: inotifyexec /tmp/modified/
"""
import errno
...
def watch_delay_call(
        ...
        mask=pyinotify.IN_CREATE | pyinotify.IN_CLOSE_WRITE | pyinotify.IN_MODIFY):
...
if __name__ == '__main__':
    ...
    parser.add_argument('source', help='the directory to copy changes from')
    parser.add_argument('target', help='the directory to copy changes to')

    args = parser.parse_args()

    source = args.source
    if not os.path.isdir(source):
        print("Error: The source directory must be a valid directory: " + source)
        sys.exit(-1)
        ...
        if paths:
            ...
            for path in set(paths):
                try:
                    ...
                    print('  Processing path {} ({}) target {} ({})'.format(path, path_mtime, target_path, target_mtime))
                    if path_mtime > target_mtime:
                        print('  Copying...'.format(path, path_mtime, target_path, target_mtime))
                        shutil.copy2(path, os.path.join(target, path, target_path))
                    else:
                        print('  Not copying due to mtime')
                except Exception as e:
                    print "ERROR: Failed to process path '{}'. Exception: {}".format(path, e)


        print('\033[36mMonitoring file changes in {0}...\033[0m'.format(os.getcwd()))


    watch_delay_call(source, callback)
